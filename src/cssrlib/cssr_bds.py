...
class cssr_bds(cssr):
    ...
    def decode_cssr_mask(self, msg, i):
        ...
        mask_bds, mask_gps, mask_gal, mask_glo = bs.unpack_from('u63u37u37u37', msg, i)
        i+=63+37+37+37
    ...
    def decode_cssr_orb_sat(self, msg, i, inet, sat_n):
        slot, iodn, iodc, dx, dy, dz, ucls, uval = bs.unpack_from('u9u10u3s15s13s13u3u3',msg,i)
        i += 69
    ...
    def decode_cssr_comb1(self, msg, i, inet=0):
        ...
        if numc>0:
            tod, _, iodssr, iodp, slot_s = bs.unpack_from('u17u4u2u4u9',msg,i)
            i+=36       
            for k in range(numc):
                idx = slot_s+k
                i = self.decode_cssr_clk_sat(msg, i, inet, idx)
