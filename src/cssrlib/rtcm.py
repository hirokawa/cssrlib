...
from cssrlib.gnss import uGNSS, sat2id, gpst2time, timediff
...
class rtcm(cssr):
    ...
    def decode_head(self, msg, i, sys):
        ...
        if self.subtype in (sCSSR.ORBIT, sCSSR.COMBINED):
            datum = bs.unpack_from('u1', msg, i)[0]
            i+=1        
    ...
    def decode_orb_sat(self, msg, i, k, sys = uGNSS.NONE, inet=0):
        ...
        i+=blen 
        dx, dy, dz, ddx, ddy, ddz = bs.unpack_from('s22s20s20s21s19s19', msg, i)
        i+=121        
    ...
    def decode_cssr_ura(self, msg, i):
        ...
        for k in range(nsat):
            ...
            cls,val = bs.unpack_from_dict('u3u3', msg, i)
            self.ura[s] = self.quality_idx(cls, val)
        ...
        for k in range(nlayer):
            ...
            for j in range(m+1):
                for l in range(j,n+1):
                     ...
                     i+=16
                     c[l,j] = self.sval(c_, 16, 5e-3)

            for j in range(1,m+1):
                for l in range(j,n+1):
                     s_ = bs.unpack_from('s16', msg, i)
                     i+=16
                     s[l,j] = self.sval(s_, 16, 5e-3)
