...
import numpy as np
from gnss import uGNSS,rCST,rSIG,prn2sat,sat2prn,Eph,gpst2time,time2gpst,sat2id,ecef2pos,geodist,satazel,ionmodel,tropmodel,dops,ecef2enu
...
def rescode(itr,obs,nav,rs,dts,svh,x):
    ...
    for i in range(n):
        ...
        if el[i]<np.deg2rad(ELMIN):
            continue
        eph=findeph(nav,obs.t,obs.sat[i])
        P=obs.P[i,0]-eph.tgd*rCST.CLIGHT
        dion=ionmodel(obs.t,pos,az[i],el[i])
...
if __name__ == '__main__':
    ...
    if dec.decode_obsh(obsfile)>=0:
        ...
        for ne in range(nep):
            ...
            dop[ne,:]=dops(az,el)
            enu[ne,:]=ecef2enu(pos0,sol[ne,0:3]-pos0)
