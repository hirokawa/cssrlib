...
def msearch(L,d,zs,m=2):
    ...
    S=np.zeros((n,n))
    dist=np.zeros(n)
    zb=np.zeros(n);z=np.zeros(n);step=np.zeros(n)
    zn=np.zeros((n,m))
    s=np.zeros(m)
    k=n-1
    zb[-1]=zs[-1]
...
if __name__ == '__main__':
    ...
    if ncase==1:    
        Qah = np.array([[6.2900, 5.9780, 0.5440],[5.9780, 6.2920, 2.3400],[0.5440, 2.3400, 6.2880]])
        ah=np.array([5.45, 3.10, 2.97])
    elif ncase==2:
        Qah=[[19068.8559508787,	-15783.9722820370,	-17334.2005875975,	14411.9239749603,	10055.7170089359,	-14259.2952903872,	14858.8484050976,	-12299.1993741839,	-13507.1694819930,	11230.0704356810,	7835.62344938376,	-11111.1393808147],\
        ...
        Qah = np.array(Qah)       
        ah=[-28490.8566886116,65752.6299198198,38830.3666554972,5003.70833517778,-29196.0699104593,-297.658932458787,-22201.0284440701,51235.8374755528,30257.7809603224,3899.40332138829,-22749.1853575113,-159.278779870217       ]
        ah = np.array(ah)
